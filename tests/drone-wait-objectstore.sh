...
function get_swift_token() {
    KEYSTONE_OUT=$(curl -s 'http://localhost:5000/v2.0/tokens' -H 'Content-Type: application/json' -d '{"auth":{"passwordCredentials":{"username":"swift","password":"swift"},"tenantName":"service"}}')
    if (echo "$KEYSTONE_OUT" | grep -q 'endpoints')
    then
